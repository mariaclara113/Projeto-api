...
const extensionista_1 = require("../schemas/extensionista");
...
router.get("/", async (req, res) => {
    try {
        const all = await prismaClient_1.prisma.extensionista.findMany();
        return res.json(all);
    }
    catch (error) {
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const item = await prismaClient_1.prisma.extensionista.findUnique({
            where: { id },
            include: { pontos: true },
        });
        if (!item)
            return res.status(404).json({ error: "Extensionista não encontrado" });
 ...
 */
router.post("/", (0, validate_1.validateBody)(extensionista_1.createExtensionistaSchema), async (req, res) => {
    try {
        const created = await prismaClient_1.prisma.extensionista.create({ data: req.body });
 ...
 */
router.put("/:id", (0, validate_1.validateBody)(extensionista_1.updateExtensionistaSchema), async (req, res) => {
    try {
        const id = Number(req.params.id);
        const updated = await prismaClient_1.prisma.extensionista.update({ where: { id }, data: req.body });
...
router.delete("/:id", async (req, res) => {
    try {
        const id = Number(req.params.id);
        await prismaClient_1.prisma.extensionista.delete({ where: { id } });
