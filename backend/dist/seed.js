...
async function main() {
    ...
    await prismaClient_1.prisma.ponto.deleteMany();
    await prismaClient_1.prisma.projeto.deleteMany();
    await prismaClient_1.prisma.extensionista.deleteMany();
    console.log("🟩 Criando extensionistas...");
    const extensionistas = await prismaClient_1.prisma.extensionista.createMany({
        data: [
    ...
    const ext1 = await prismaClient_1.prisma.extensionista.findFirst({ where: { email: "joao@example.com" } });
    const ext2 = await prismaClient_1.prisma.extensionista.findFirst({ where: { email: "maria@example.com" } });
    const ext3 = await prismaClient_1.prisma.extensionista.findFirst({ where: { email: "carlos@example.com" } });
    console.log("🟧 Criando projetos...");
    const projeto1 = await prismaClient_1.prisma.projeto.create({
        data: {
            nome: "Projeto Comunidade",
            descricao: "Projeto de apoio à comunidade local",
        }
    });
    const projeto2 = await prismaClient_1.prisma.projeto.create({
        data: {
    ...
    await prismaClient_1.prisma.ponto.createMany({
        data: [
            {
                dataHora: new Date(),
                tipo: "ENTRADA",
